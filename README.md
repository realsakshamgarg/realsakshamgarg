...
# CONFIG
# ----------------------------------------------------------------------------
USERNAME = "SakshamGarg"

def load_image_data():
    f = Path(__file__).parent / IMAGE_FILE
    if not f.exists():
...
INFO = [
    ...
    ("Edu",      "B.Tech CSE (Data Science) - SRM University · Class of 2028", "val"),
    ("Focus",    "Data Analysis · Data Science · Machine Learning", "accent"),
    ("__blank__", "", ""),
    ("__section__", "~/stack", ""),
    ("Lang",     "Python · SQL · HTML · CSS · JavaScript", "val"),
    ("Backend",  "FastAPI · Flask · Node.js · REST · Microservices", "val"),
    ("AI",       "LLMs · RAG · LangChain · CrewAI · Vector DBs", "val"),
    ("__blank__", "", ""),
    ("__section__", "~/projects", ""),
    ("Project",  "CrewAI - AI-powered SaaS for businesses", "val"),
    ("__blank__", "", ""),
    ("__section__", "~/highlights", ""),
    ("Award",    "Aspire Leadership Program 2026", "val"),
    ("__stats__", "", ""),
    ("__blank__", "", ""),
    ("__section__", "~/reach", ""),
    ("Portfolio",      "realsakshamgarg.in", "accent"),
    ("LinkedIn",       "linkedin.com/in/realsakshamgarg", "accent"),
    ("GMail",     "sakshamgarg1303@gmail.com", "accent");
      ]
...
def render(theme_name, colors, stats, ist_now):
    ...
    # command line
    parts.append(
        f'<text x="{IMG_X}" y="66" class="row" style="animation-delay:.05s">'
    ...
    parts.append(
        f'<image x="{IMG_X}" y="{IMG_Y}" width="{IMG_WIDTH}" height="{IMG_HEIGHT}" '
        f'preserveAspectRatio="xMidYMid slice" href="{img_data}" />'
    )
    ...
    fy = H - 24
    parts.append(
        f'<text x="{IMG_X}" y="{fy}" class="row" style="animation-delay:{delay+0.1:.2f}s">'
        f'<tspan class="key">➜</tspan>'
        f'<tspan class="acc" dx="8">~</tspan>'
